...
# No rule can name a reverse zone, so a row for one could never be taken away
# by writing a rule. The resolver records it under a verb of its own instead.
if grep -qF "in-addr.arpa" <<< "$SUMMARY"; then

...
export function generateCorednsConfig(options: CorednsConfigOptions): GeneratedCorednsConfig {
  ...
  for (const rule of options.httpsRules ?? []) add(hostRegexOfHostRule(rule));
  for (const rule of options.httpRules ?? []) add(hostRegexOfHostRule(rule));
  for (const rule of options.tlsRules ?? []) add(hostRegexOfHostRule(rule));
  for (const rule of options.urlRules ?? []) add(hostRegexOfUrlRule(rule));

...
- **Sigstore has to be reachable.** Verification depends on the Rekor transparency log and the
  ...
  rather than skipping the check. Each fetch starts from the root embedded in the action, in a
  directory of its own, so no trust root an earlier job left on a persistent runner is used.
